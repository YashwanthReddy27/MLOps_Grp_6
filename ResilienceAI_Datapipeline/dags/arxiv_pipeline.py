...
ARXIV_KEYWORDS = {
    'artificial_intelligence': {
        'keywords': ['artificial intelligence', 'machine learning', 'deep learning', 'neural networks', 'GPT', 'LLM', 'AI', 'generative AI', 'transformer', 'reinforcement learning'],
        'weight': 1.0
    }
}
...
def fetch_arxiv_papers(**context):
    ...
    try:
        ...
        dedup = DeduplicationManager('arxiv')
        
        # Ensure directories exist
        file_mgr.ensure_directories()
        
        print(f"[EXTRACT] Fetching arXiv papers for AI research")
        ...
        print(f"[EXTRACT] Query: {category_query}")
        print(f"[EXTRACT] Date range: {start_date.date()} to {end_date.date()}")
        ...
        for entry in entries:
            try:
                ...
                print(f"[EXTRACT] Error parsing entry: {e}")
                continue
        
        print(f"[EXTRACT] Successfully parsed {len(papers)} papers")
        ...
        print(f"[EXTRACT] New unique papers: {len(new_papers)}")
        print(f"[EXTRACT] Duplicates filtered: {len(papers) - len(new_papers)}")
        ...
        if new_papers:
            ...
            output_data = {
                ...
                'keywords_config': ARXIV_KEYWORDS,
            ...
            print(f"[EXTRACT] Successfully saved {len(new_papers)} new papers to {filename}")
            ...
            print("[EXTRACT] No new papers found. All papers were duplicates.")
            context['task_instance'].xcom_push(key='result', value={
                'filename': None,
                'paper_count': 0,
        ...
        print(f"[EXTRACT] Error fetching arXiv data: {str(e)}")
        raise
    except Exception as e:
...
def process_and_categorize_papers(**context):
    """
    ...
    try:
        ...
        print(f"[PROCESS] Processing file: {raw_file}")
        ...
        for paper in papers:
            ...
            if not validator.validate_arxiv_id(arxiv_id):
                print(f"[PROCESS] Invalid arXiv ID after cleaning: {arxiv_id_raw} -> {arxiv_id}")
                continue
            ...
            categorization = CategoryManager.categorize_content(combined_text, ARXIV_KEYWORDS)
        ...
        print(f"[PROCESS] Processed {len(processed_papers)} papers")
        
        # Print category distribution
        category_counts = CategoryManager.get_category_distribution(processed_papers)
        print("[PROCESS] Category distribution:")
...
def load_to_postgresql(**context):
    ...
    try:
        ...
        print("[DB] Table created/verified successfully")
        ...
        if not processed_result or not processed_result.get('filename'):
            print("[DB] No new papers to load")
            ...
            try:
                ...
                if existing_papers:
                    print(f"[DB] Found {len(existing_papers)} existing papers in database")
                ...
                print(f"[DB] Error querying existing papers: {e}")
                context['ti'].xcom_push(key='result', value={
                    'status': 'no_papers',
        ...
        print(f"[DB] Loading new data from: {processed_file}")
        ...
        print(f"[DB] Successfully loaded {count} NEW papers to PostgreSQL")
...
def cleanup_old_files(**context):
    ...
    try:
        ...
        print(f"[CLEANUP]   - Processed files: {deleted_processed}")
...
with DAG(
    ...
    description='Fetch and categorize AI research papers from arXiv',
    schedule_interval='0 0 * * *',  # Run daily at midnight
    start_date=days_ago(1),
    catchup=False,
    tags=['arxiv', 'research', 'ai', 'papers', 'automated'],
) as dag:
    ...
    fetch_papers_task >> process_task >> load_db_task >> cleanup_task